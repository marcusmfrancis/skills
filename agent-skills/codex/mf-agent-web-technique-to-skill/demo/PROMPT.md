...
```text
Use $mf-agent-web-technique-to-skill to extract the mechanism behind this effect and package it as a reusable skill with a demo.
```

## Recreate the demo

Use `$mf-agent-web-technique-to-skill` to build **The Triage Bench** as a single standalone HTML document. Treat `index.html` as the visual, motion, responsive, accessibility, and performance reference.
