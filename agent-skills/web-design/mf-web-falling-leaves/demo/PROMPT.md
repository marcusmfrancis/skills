...
```text
Use $mf-web-falling-leaves to add a maple fall over this section, with leaves that tumble edge-on rather than a generic particle field.
```

## Recreate the demo

Use `$mf-web-falling-leaves` to build **Late Autumn — The Maple Fall** as a single standalone HTML document. Treat `index.html` as the visual, motion, responsive, accessibility, and performance reference.
