...
```text
Use $mf-web-pointer-trail-emitter to add a mote trail to this hero that emits by distance travelled, so the spacing holds whether the hand crawls or flicks.
```

## Recreate the demo

Use `$mf-web-pointer-trail-emitter` to build **Wisps** as a focused local demo. Treat `index.html` as the visual, motion, responsive, accessibility, and performance reference.
